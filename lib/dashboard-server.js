  ...
  if (request.method === 'POST' && pathname === '/api/sync-daily') {
    return authenticate(request, response, db, async (operatorId, operator) => {
      try {
        ...
        const syncPromise = runSyncDaily({ db, date: targetDate, targetCA })
          .finally(() => {
