...
function parseResumeText(rawText) {
  ...
  const workBlocks = extractWorkBlocks(rawText);
  ...
  if (/ph\.?d|doctorate|doctor of/i.test(rawText)) {
    highestEducation = "Doctorate";
  } else if (/master(?:'s)?|ms |m\.s\.|mba|m\.sc/i.test(rawText)) {
    highestEducation = "Master's Degree";
  } else if (/bachelor(?:'s)?|bs |b\.s\.|btech|b\.tech|b\.e\.|ba /i.test(rawText)) {
    highestEducation = "Bachelor's Degree";
  } else if (/associate(?:'s)?/i.test(rawText)) {
